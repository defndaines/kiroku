#!/bin/bash
# Reports average rating per country across all *-reading.md tables,
# double-counting a book across every country it lists. Sorted by average
...
#
# Usage: average-rating-by-country.sh [-t|--threshold N] [-x|--exclude-graphic-novels]
...
(echo "Country|Count|Average"
awk -F' *\\| *' -v exclude_gn="$exclude_graphic_novels" '
FNR == 1 { header_seen = 0; ccol = 0; rcol = 0; tcol = 0 }
!header_seen {
  if ($0 !~ /[A-Za-z0-9]/) next
  for (i = 1; i <= NF; i++) {
    field = tolower($i)
    if (field == "country") ccol = i
    if (field == "rating") rcol = i
    if (field == "tags" || field == "genre") tcol = i
  }
  if (ccol && rcol) header_seen = 1
  next
}
$0 ~ /---/ { next }
{
  if (exclude_gn && tcol && $tcol ~ /graphic novel/) next
...
END {
  for (country in count)
    printf "%s|%d|%.2f\n", country, count[country], sum[country] / count[country]
}
